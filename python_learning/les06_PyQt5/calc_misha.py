...
class Window(QMainWindow, Ui_Form):
    def __init__(self):
        ...
        for btn in (self.btn0, self.btn1, self.btn2, self.btn3,
                    ...
                    self.btn8, self.btn9, self.btn_clear, self.btn_div,
                    self.btn_plus, self.btn_dot, self.btn_eq, self.btn_fact,
                    self.btn_minus, self.btn_mult, self.btn_pow,
                    self.btn_sqrt):
    ...
    def run(self):
        opers = {'+': lambda a, b: a + b,
                 ...
                 '^': lambda a, b: a ** b if not (a == 0 and b < 0) else 0,
                 '!': lambda x: math.factorial(x) if x >= 0 and
                 type(x) is int else 0,
                 '√': lambda x: x ** (1 / 2)}
        btn = self.sender().text()  # type: ignore
        if btn in '0123456789':
            ...
            self.is_float = False
            if btn in '!√':
                self.res = str(self.op(int(self.res)))
                self.mem = ''
        elif btn == '=':
            if self.mem:
                if not self.is_pressed:
                    self.res = str(self.op(float(self.mem), float(self.res)))
            self.is_pressed = True
            self.eq_is_pressed = True
        elif btn == 'С':
            self.res = '0'
            if self.eq_is_pressed:
                self.mem = ''
                self.eq_is_pressed = False
            self.is_float = False
            self.is_pressed = True
        ...
        self.table.display(self.res)  # type: ignore
